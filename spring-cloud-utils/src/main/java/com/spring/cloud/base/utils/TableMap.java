...
public class TableMap<K, V> implements Map<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
	...
	private static final long serialVersionUID = -7815128310805831702L;
	...
	public boolean remove(final Object key, final Object value) {
		...
		for (int i = 0; i < size(); i++) {
			if (ObjectUtil.equals(key, keys.get(i)) && ObjectUtil.equals(value, values.get(i))) {
				removeByIndex(i);
				removed = true;
				// 移除当前元素，下个元素前移
				i--;
	...
	public boolean replace(final K key, final V oldValue, final V newValue) {
		for (int i = 0; i < size(); i++) {
			if (ObjectUtil.equals(key, keys.get(i)) && ObjectUtil.equals(oldValue, values.get(i))) {
				values.set(i, newValue);
				return true;
			}
		}
	...
	public V replace(final K key, final V value) {
		V lastValue = null;
		for (int i = 0; i < size(); i++) {
			if (ObjectUtil.equals(key, keys.get(i))) {
