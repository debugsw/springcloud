...
import com.spring.cloud.base.http.global.GlobalHeaders;
            ...
            if (responseCode != HttpURLConnection.HTTP_OK) {
                if (HttpStatus.isRedirected(responseCode)) {
                    ...
                    if (false == HttpUtil.isHttp(location) && false == HttpUtil.isHttps(location)) {
                        ...
                        redirectUrl = UrlBuilder.of(this.url.getScheme(), this.url.getHost(), this.url.getPort(), location, null, null, this.charset);
                    } else {
                    ...
                    if (redirectCount < config.maxRedirectCount) {
                        redirectCount++;
                        // 重定向不再走过滤器
                        return doExecute(isAsync, config.interceptorOnRedirect ? config.requestInterceptors : null, config.interceptorOnRedirect ? config.responseInterceptors : null);
                    }
                }
            }
        }
